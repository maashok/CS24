...
/* This is a mask used in finding various parts of an address */
#define oneMask 0xFFFFFFFF
...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    p_cache->num_reads++;
    /* Update the last access time of the cache line */
    p_line->recentUse = clock_tick();
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    /* The number of bits for offset and set number are in the cache */
    addr_t bitsOffset = p_cache->block_offset_bits;
    addr_t bitsSet = p_cache->sets_addr_bits;

    /* The tag is the first set of bits */
    *tag = address >> (bitsOffset + bitsSet);

    /* The offset is the last set of bits */
    addr_t mask1 = oneMask >> (32 - bitsOffset);
    *offset = (address & mask1);

    /* The set is in the middle, and is found by masking
     * the top of the address first and then shifting 
     * to hide the end of the address */
    addr_t mask2 = oneMask >> (32 - bitsOffset - bitsSet);
    *set = ((address & mask2) >> bitsOffset);
}
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
  /* The block start has the same tag and set number, but the offset
   * is zero */
  addr_t intermediate = address >> (p_cache->block_offset_bits);
  return intermediate << p_cache->block_offset_bits;
}
 ...
 */
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
  /* The offset is found by decomposing the address */
  addr_t tag = 0;
  addr_t set = 0;
  addr_t offset = 0;

  /* Decompose the address and get the offset part */
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
  ...
  /* The tag part of the address is in the start of the address */
  addr_t tagPart = tag << (bitsOffset + p_cache->sets_addr_bits);
  
  /* The set part of the address is in the middle of the address */
  addr_t setPart = set_no << (bitsOffset);
  
  /* The offset will be zero for the start of the address, so
   * just put together the previous 2 parts */
  addr_t start = tagPart | setPart;
...
#endif
    int i;
    /* Go through each of the lines in the cache set.
     * Find the line with the same tag, that is also flagged as valid */
...
#else
    i_victim = 0;
    /* Set the most recently used to the first line at first */
    unsigned long long int smallestAccess = (p_set->cache_lines)->recentUse;
    int i;
    /* Go through the lines in the set */
    for (i = 0; i < p_set->num_lines; i++) {
      /* If a line is not valid (meaning it's empty), it is evicted */
      if (!((p_set->cache_lines + i)->valid)) {
        i_victim = i;
        break;
      }
      
      /* Otherwise, if this line was less recently used than
       * the smallest so far, make it the smallest */
      if ((p_set->cache_lines + i)->recentUse < smallestAccess) {
        i_victim = i;
        smallestAccess = (p_set->cache_lines + i)->recentUse;
      }  
    }
    /* The victim is the line that was least recently used, or
     * is invalid */
    victim = p_set->cache_lines + i_victim;
