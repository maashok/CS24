...
typedef enum {
    ...
    /*!
     * The thread is ready to run, but doesn't have access to the CPU yet.  
     * The thread will be kept in the ready-queue.
     */
    ThreadReady,
    ...
    /*!
     * The thread's function has returned, and therefore the thread is ready 
     * to be permanently removed from the scheduling mechanism and 
     * deallocated.
     */
    ThreadFinished
} ThreadState;
 ...
 */
void __sthread_finish(void) {
  /* Lock the thread so that the entire operation (including the schedule) is
   * atomic. That way, no other interrupt can happen between when the state
   * is changed and when a new thread is scheduled, causing a thread to 
   * continue to be used when it should not be */
    __sthread_lock();
 ...
 */
void sthread_yield() {
  /* Lock the thread before calling __sthread_schedule() to ensure
   * that the schedule function is atomic and the setting up and restoring
   * of the context are not interrupted in between. If they are, the
   * stack manipulations as well as the queue manipulations in 
   * __sthread_scheduler will be affected */
    __sthread_lock();
    __sthread_schedule();
}
 ...
 */
void sthread_block() {
  /* Lock the thread so that the entire operation of blocking is atomic
   * That way, nothing will interrupt the thread between changing the
   * state to blocked and scheduling a new thread, so that we ensure
   * that the thread is added to the blocked queue immediately after
   * changing its state. Also, we want the scheduling operation to
   * be atomic to ensure that the stack manipulaitons are not interrupted
   */
   __sthread_lock();
...
void sthread_unblock(Thread *threadp) {
    /* Lock the thread to ensure that the manipulation of the
     * blocked and ready queues are not interrupted, which could
     * cause the queues to be incompletely changed, or changed
     * by multiple threads simultaneously. This could lead to 
     * unpredictable behavior based on when the thread is interrupted
     */
    __sthread_lock();
    ...
    threadp->state = ThreadReady;
    queue_add(threadp);
    /* Unlock the thread to allow other threads to start running
     * at any point after the unblocking is over. This way, other
     * threads will get a chance to run as well in other non interrupt
     * sensitive functions */
    __sthread_unlock();
}
