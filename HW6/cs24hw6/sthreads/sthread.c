...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
  /* If the current thread is not NULL, set its context and
   * examine the state of it */
  if (current != NULL) {
  ...
  /* If the ready queue is not empty, take a thread from
   * the queue to run */
  if (!queue_empty(&ready_queue)) {
    ...
    /* Otherwise,  if there are no blocked threads either,
     * all threads have completed */
    if (queue_empty(&blocked_queue)) {
       printf("All threads have completed successfully \n");
       exit(0);
     }
    /* If there are blocked threads still there, there is
     * deadlock */
     else {
       printf("The program is deadlocked!\n");
       exit(1);
     }
  }
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
  /* Allocate memory for the stack of the thread, and the thread itself */
  ThreadContext *stack = malloc(DEFAULT_STACKSIZE);

  Thread *thread = malloc(sizeof(Thread));

  /* The thread starts out ready */
  thread->state = ThreadReady;
  thread->memory = stack;

  queue_add(thread);
  /* Initialize the context of the thread with the function to perform
   * afterwards */
  thread->context = (ThreadContext *)
    __sthread_initialize_context((char *)stack + DEFAULT_STACKSIZE, f, arg);
 ...
 */
void __sthread_delete(Thread *threadp) {
  /* Deleting the stack memory of the thread will delete the context also */
  free(threadp->memory);
  free(threadp);
}
